...
export function useCodeBlocks(containerRef: Ref<HTMLElement | undefined>) {
  ...
  // 获取代码内容
  const getCodeContent = (wrapper: Element): string => {
  ...
  // 复制辅助函数
  const copyToClipboard = async (code: string, btn: Element) => {
    try {
  ...
  // 增强代码块：添加复制按钮功能（对于无头模式还需添加语言标签）
  const enhanceCodeBlocks = () => {
    if (!containerRef.value) return;
    const wrappers = containerRef.value.querySelectorAll(".code-block-wrapper");

    wrappers.forEach((wrapper) => {
      if (wrapper.hasAttribute("data-enhanced")) return;
      const hasHeader = wrapper.classList.contains("has-header");
      const pre = wrapper.querySelector("pre");
      if (!pre) return;

      const code = getCodeContent(wrapper);
      const lang = pre.getAttribute("data-language") || "text";

      if (hasHeader) {
        // 有头部：复制按钮已经存在于 .tab-actions 中，只需绑定事件
